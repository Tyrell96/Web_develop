...
$(document).ready(function(e) { 
	$(".check").on("keyup", function(){ //check라는 클래스에 입력을 감지
		var self = $(this); //this는 모든 class를 선택하면 문제가 생기기 때문에 클릭한 클래스만 누르면 작동하도록 한다.
		var userid; 
		
		if(self.attr("id") === "userid"){   //===는 변수의 유형 고려 == 는 변수 값만 기반 
			userid = self.val(); 
		} 
		
		$.post( //post방식으로 id_check.php에 입력한 userid값을 넘긴다.
			"id_check.php",
			{ userid : userid }, 
			function(data){ 
